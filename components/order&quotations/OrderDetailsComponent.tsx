...
export const OrderDetailsComponent: React.FC<OrderDetailsComponentProps> = ({
  ...
  const { data: leadDetails } = useLeadDetails(order?.lead_id || '');
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.root}>
      ...
      >
        ...
        <View style={styles.detailsCard}>
          <View style={styles.detailsGrid}>
            <View style={styles.detailsRow}>
              <Ionicons name="document-text-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
              <View style={styles.detailsCol}>
                <Text style={styles.detailsLabel}>ORDER NUMBER</Text>
                <Text style={styles.detailsValue}>{order.orderNo || `OD-${order.order_number}` || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailsRow}>
              ...
              <View style={styles.detailsCol}>
                ...
                <Text style={styles.detailsValue}>{order.customer_name || order.contactPerson || order.clientName || 'N/A'}</Text>
              </View>
            </View>

            <View style={styles.detailsRow}>
              <Ionicons name="options-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
              <View style={styles.detailsCol}>
                <Text style={styles.detailsLabel}>ORDER TYPE</Text>
                <Text style={styles.detailsValue}>{(order.order_type || 'N/A').toUpperCase().replace(/_/g, ' ')}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailsRow}>
              ...
              <View style={styles.detailsCol}>
                ...
                <Text style={styles.detailsValue}>{order.date || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailsRow}>
              ...
              <View style={styles.detailsCol}>
                ...
                (order.status || 'DRAFT').toUpperCase() === 'COMPLETED' || (order.status || 'DRAFT').toUpperCase() === 'DELIVERED' || (order.status || 'DRAFT').toUpperCase() === 'CONFIRMED'
                  ? styles.scannedBadge
                  ...
                  <Text style={[styles.statusBadgeText, {
                    color: (order.status || 'DRAFT').toUpperCase() === 'COMPLETED' || (order.status || 'DRAFT').toUpperCase() === 'DELIVERED' || (order.status || 'DRAFT').toUpperCase() === 'CONFIRMED'
                      ? '#03543F'
                      : (order.status || 'DRAFT').toUpperCase() === 'CANCELLED'
                        ? '#991B1B'
                        : '#92400E'
            ...
            <View style={styles.detailsRow}>
              <Ionicons name="card-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
              <View style={styles.detailsCol}>
                <Text style={styles.detailsLabel}>PAYMENT STATUS</Text>
                <View style={[styles.statusBadge, { alignSelf: 'flex-start', marginTop: 2 },
                (order.payment_status || 'PENDING').toUpperCase() === 'PAID' || (order.payment_status || 'PENDING').toUpperCase() === 'CONFIRMED'
                  ? styles.scannedBadge
                  : styles.pendingBadge
                ]}>
                  <Text style={[styles.statusBadgeText, {
                    color: (order.payment_status || 'PENDING').toUpperCase() === 'PAID' || (order.payment_status || 'PENDING').toUpperCase() === 'CONFIRMED'
                      ? '#03543F'
                      : '#92400E'
                  }]}>
            ...
            <View style={styles.detailsRow}>
              ...
              <View style={styles.detailsCol}>
                ...
                <Text style={styles.detailsValue}>{(order.sales_member_name || order.sales_representative || order.approvedBy || 'N/A').toUpperCase()}</Text>
              </View>
            </View>

            <View style={styles.detailsRow}>
              <Ionicons name="person-circle-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
              <View style={styles.detailsCol}>
                <Text style={styles.detailsLabel}>CREATED BY</Text>
                <Text style={styles.detailsValue}>{(order.created_by_name || 'N/A').toUpperCase()}</Text>
              </View>
            </View>

            <View style={styles.detailsRow}>
              <Ionicons name="bus-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
              <View style={styles.detailsCol}>
                <Text style={styles.detailsLabel}>TRACKING NUMBER</Text>
                <Text style={styles.detailsValue}>{order.tracking_number || 'N/A'}</Text>
              </View>
            </View>
            ...
            {order.expected_delivery_date && (
              <View style={styles.detailsRow}>
                <Ionicons name="time-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
                <View style={styles.detailsCol}>
                  <Text style={styles.detailsLabel}>EXPECTED DELIVERY</Text>
                  <Text style={styles.detailsValue}>{new Date(order.expected_delivery_date).toLocaleDateString('en-IN')}</Text>
                </View>
              </View>
            )}
            ...
            {order.actual_delivery_date && (
              <View style={styles.detailsRow}>
                <Ionicons name="checkmark-done-circle-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
                <View style={styles.detailsCol}>
                  <Text style={styles.detailsLabel}>ACTUAL DELIVERY</Text>
                  <Text style={styles.detailsValue}>{new Date(order.actual_delivery_date).toLocaleDateString('en-IN')}</Text>
                </View>
              </View>
            )}

            {order.sales_member_order_sequence && (
              <View style={styles.detailsRow}>
                <Ionicons name="list-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
                <View style={styles.detailsCol}>
                  <Text style={styles.detailsLabel}>SEQUENCE NO</Text>
                  <Text style={styles.detailsValue}>{order.sales_member_order_sequence}</Text>
                </View>
              </View>
            )}

            {order.stages_total > 0 && (
              <View style={styles.detailsRow}>
                <Ionicons name="git-branch-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
                <View style={styles.detailsCol}>
                  <Text style={styles.detailsLabel}>STAGES COMPLETED</Text>
                  <Text style={styles.detailsValue}>{order.stages_completed} / {order.stages_total}</Text>
                </View>
              </View>
            )}

            {order.billing_address && (
              <View style={[styles.detailsRow, { width: '100%' }]}>
                <Ionicons name="receipt-outline" size={15} color={theme.primaryColor} style={styles.detailsIcon} />
                <View style={styles.detailsCol}>
                  <Text style={styles.detailsLabel}>BILLING ADDRESS</Text>
                  <Text style={[styles.detailsValue, { lineHeight: 18 }]}>{order.billing_address}</Text>
                </View>
              </View>
            )}
            ...
            <View style={[styles.detailsRow, { width: '100%', borderBottomWidth: 0, paddingBottom: 0 }]}>
              ...
              <View style={styles.detailsCol}>
                ...
                <Text style={[styles.detailsValue, { lineHeight: 18 }]}>
                  {order.shipping_address_line1 || order.hotelLocation || 'N/A'}
                </Text>
              </View>
            </View>
          </View>
        ...
        {(order.internal_notes || order.customer_notes || order.reject_remarks || order.cancellation_reason) && (
          <View style={[styles.detailsCard, { marginTop: 8 }]}>
            <Text style={[styles.detailsLabel, { marginBottom: 6, fontSize: 10, letterSpacing: 0.5 }]}>REMARKS & NOTES</Text>
            {order.internal_notes && (
              <View style={{ marginBottom: 6 }}>
                <Text style={{ fontSize: 10.5, fontWeight: '800', color: COLORS.textMuted }}>INTERNAL NOTES</Text>
                <Text style={{ fontSize: 12, fontWeight: '700', color: COLORS.textDark, marginTop: 1 }}>{order.internal_notes}</Text>
              </View>
            )}
            {order.customer_notes && (
              <View style={{ marginBottom: 6 }}>
                <Text style={{ fontSize: 10.5, fontWeight: '800', color: COLORS.textMuted }}>CUSTOMER NOTES</Text>
                <Text style={{ fontSize: 12, fontWeight: '700', color: COLORS.textDark, marginTop: 1 }}>{order.customer_notes}</Text>
              </View>
            )}
            {order.reject_remarks && (
              <View style={{ marginBottom: 6 }}>
                <Text style={{ fontSize: 10.5, fontWeight: '800', color: COLORS.danger }}>REJECT REMARKS</Text>
                <Text style={{ fontSize: 12, fontWeight: '700', color: COLORS.danger, marginTop: 1 }}>{order.reject_remarks}</Text>
              </View>
            )}
            {order.cancellation_reason && (
              <View style={{ marginBottom: 6 }}>
                <Text style={{ fontSize: 10.5, fontWeight: '800', color: COLORS.danger }}>CANCELLATION REASON</Text>
                <Text style={{ fontSize: 12, fontWeight: '700', color: COLORS.danger, marginTop: 1 }}>{order.cancellation_reason}</Text>
              </View>
            )}
          </View>
        )}
        ...
        <View style={styles.billSummaryCard}>
          <View style={styles.billSummaryRow}>
            ...
            <Text style={styles.billSummaryVal}>₹{(order.subtotal || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</Text>
          </View>
          {order.discount_amount > 0 && (
            <View style={styles.billSummaryRow}>
              <Text style={styles.billSummaryLabel}>
                Discount {order.discount_type === 'PERCENTAGE' ? `(${order.discount_value || 0}%)` : ''}
              </Text>
              <Text style={[styles.billSummaryVal, { color: COLORS.danger }]}>
                - ₹{(order.discount_amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
              </Text>
            </View>
          )}
          {order.gst_amount > 0 && (
            <View style={styles.billSummaryRow}>
              <Text style={styles.billSummaryLabel}>Estimated GST</Text>
              <Text style={styles.billSummaryVal}>₹{(order.gst_amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</Text>
            </View>
          )}
          {order.shipping_charges > 0 && (
            <View style={styles.billSummaryRow}>
              <Text style={styles.billSummaryLabel}>Shipping / Freight</Text>
              <Text style={styles.billSummaryVal}>₹{(order.shipping_charges || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</Text>
            </View>
          )}
          {order.additional_charges > 0 && (
            <View style={styles.billSummaryRow}>
              <Text style={styles.billSummaryLabel}>Additional Charges</Text>
              <Text style={styles.billSummaryVal}>₹{(order.additional_charges || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</Text>
            </View>
          )}
          <View style={[styles.billSummaryRow, styles.billSummaryGrand]}>
            <Text style={styles.grandLabel}>Payable Amount</Text>
            <Text style={[styles.grandVal, { color: theme.primaryColor }]}>{order.amount}</Text>
          </View>
          {order.advance_amount > 0 && (
            <View style={styles.billSummaryRow}>
              <Text style={styles.billSummaryLabel}>Advance Paid</Text>
              <Text style={[styles.billSummaryVal, { color: '#0F766E' }]}>
                ₹{(order.advance_amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
              </Text>
            </View>
          )}
